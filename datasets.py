...
def load_img_path1(dataset_path, is_train=True, model=None):
    A_path = []
    B_path = []
    L_path = []
    is_train_val = True
    ...
    if is_train:
        if model == "SC":
            ...
            for line in f3.readlines():
                L_path.append(line.strip())
            f3.close()
    f4 = open(os.path.join(dataset_path, "pesudo.txt"), 'r')
    for line in f4.readlines():
        P_path.append(line.strip())
    f4.close()
    return A_path, B_path, L_path,P_path
...
class ImageDataset(Dataset):
    def __init__(self, root, transforms_=None, transforms_L=None, is_train=True, model=None):
        ...
        self.list_A, self.list_B, self.list_L= load_img_path1(dataset_path=root, is_train=is_train,model=model)
        # self.list_A, self.list_B, self.list_L,self.list_P = load_img_path(dataset_path=root, is_train=is_train, model=model)
        # print("---------------------------------")
    ...
    def __getitem__(self, index):
        ...
        # pesudo = Image.open(self.list_P[index]).convert('P')
        # return self.transform(img1), self.transform(img2), np.array(img1), np.array(img2), labl, name
        # return self.transform(img1), self.transform(img2), np.array(img1), np.array(img2), self.transforml(labl),self.transforml(pesudo), name
